...
class TournamentOrdersExport implements FromQuery, WithHeadings, ShouldAutoSize
{
    ...
    public function query()
    {
        ...
        ->leftJoin('votes as v', 'v.user_id', '=', 'u.id')
        ->where('u.role_id', 2)->whereNotNull('u.email_verified_at')
        ->leftJoin('contest_categories as cc', 'cc.id', '=', 'v.contest_category_id')
        ->where('cc.contest_id', '=', $this->contest_id)
        ->select('u.name', 'u.email', 'u.created_at')->distinct('u.email')->orderBy('u.name', 'ASC');
        //return User::query()->select('name', 'email', 'created_at')->where('role_id', 2)->whereNotNull('email_verified_at');*/
    }

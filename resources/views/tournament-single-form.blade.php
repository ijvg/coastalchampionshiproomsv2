...
@section('content')
            ...
            @php $roomsTypesAvailable = 0; $roomNameIfOneOnly = ""; @endphp
            ...
            <div id="homeOuterCon">
                ...
                <div class="container">
                    ...
                    <div class="hotelImagesAndInfoCon">
                        ...
                        <h3 class="teamMemberHotelName">{{ $hotel->name }}</h3>

                        <div class="row">
                            <div class="col-md-6 teamMemberFormHotelImageCon">
                                <img id="teamMemberFormHotelImage" src="{{ Voyager::image( $hotel->image ) }}">
                            </div>
                            <div class="col-md-6 teamMemberFormHotelInfo">
                                {!! $hotel->description !!}
                            </div>
                        </div>
                    ...
                    <h2 style="text-align: center; margin-top: 25px;">Single Participant's Room Form</h2>
                    ...
                    <form action="/requests/single-submit-request" class="needs-validation" novalidate id="sr_application_form" method="POST" style="margin-bottom: 0px;">
                        ...
                        @foreach( $hotelTournamentRooms as $TournamentHotelRoom)
                            @if ( $TournamentHotelRoom->rooms_available >= 1)

                                @php $roomNameIfOneOnly = "room-" . $TournamentHotelRoom->id;@endphp
                                {{-- <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label class="control-label">{{ $TournamentHotelRoom->hotelRoomtype->type }} - Price Per Night ${{ number_format($TournamentHotelRoom->gross_price_per_night,2, '.', '') }} - Rooms Available for your team: {{ $TournamentHotelRoom->rooms_available }}</label>
                                    <input class="form-control roomsInput" data-request-room-id="{{ $TournamentHotelRoom->id }}" data-price-per-night="{{ $TournamentHotelRoom->gross_price_per_night }}" type="number" max="{{ $TournamentHotelRoom->rooms_available }}" name="room-{{ $TournamentHotelRoom->id }}" value="0">
                                </div>
                                <!--</div>-->

                                <input type="hidden" name="total-cost-room-{{ $TournamentHotelRoom->id }}" value="">
                            @endif
                        @endforeach
                        ...
                        @else --}}
                            {{--<input type="hidden" name="checkInDate" value="{{ $tournament->default_check_in }}">
                            <input type="hidden" name="checkOutDate" value="{{ $tournament->default_check_out }}">--}}

                        @if ( $hotelTournament[0]->custom_nights_stay == 0)
                            <input type="hidden" name="checkInDate" value="{{ $tournament->default_check_in }}">
                            <input type="hidden" name="checkOutDate" value="{{ $tournament->default_check_out }}">
                            
                            <label style="font-weight: 500; font-size: 1.4rem;" for="numberOfNights" class="control-label" id="numberOfNightsLabel"><span>{{ $hotelTournament[0]->min_nights_stay }}</span> nights</label>
                            <input type="hidden" name="numberOfNights" value="{{ $hotelTournament[0]->min_nights_stay }}">
                        @else

                            <div class="form-group row g-3">
                                <div class="col-md-6">
                                    <label for="checkInDate" class="control-label">Check In Date</label>
                                    <input type="text" class="form-control" id="checkInDatepicker" name="checkInDate" value="{{ $tournament->default_check_in }}">
                                </div>

                                <div class="col-md-6">
                                    <label for="checkOutDate" class="control-label">Check Out Date</label>
                                    <input type="text" class="form-control" id="checkOutDatepicker" name="checkOutDate" value="{{ $tournament->default_check_out }}">
                                </div>
                            ...
                            <h5>Check-in: {{ $tournament->default_check_in }}</h5>
                            <h5>Check-out: {{ $tournament->default_check_out }}</h5>
                            {{--<h5>Number of Nights: {{ $tournament->number_of_nights }}</h5> --}}
        ...
        @foreach( $hotelTournamentRooms as $requestRoom) 
            @php if ($trrgCount == 0) { $trRoomGroup .= "room-" . $requestRoom->id; } else { $trRoomGroup .= " room-" . $requestRoom->id; } $trrgCount++; @endphp
        @endforeach
...
<script defer>
    ...
    document.addEventListener("DOMContentLoaded", function(){
        ...
        var flatFeePerNight = {{ $hotel->flat_fee_per_night }};
        var hotelTax = {{ $hotel->tax_fee_per_night }};
        ...
        var checkInMin = new Date("{{ $tournament->default_check_in }}".replace(/-/g, ","));
        var checkInMax = new Date("{{ $tournament->default_check_out }}".replace(/-/g, ','));

        checkInMax.setDate(checkInMax.getDate() - 1);

        var checkOutMin = new Date(checkInMax);
        checkOutMin.setDate(checkInMin.getDate() + 1);

        //var testDate = new Date("2021,11,18");

        console.log(checkInMin.getDate() + 1);
        ...
        $("#checkInDatepicker").datepicker({
            ...
            onSelect: function(date){
                ...
                var selectedDate = new Date(date.replace(/-/g, ","));
                var checkOutMinDate = new Date(checkInMax);
                checkOutMinDate.setDate(selectedDate.getDate() + 1);
                console.log(checkOutMinDate);
                $("#checkOutDatepicker").datepicker("option", "minDate", checkOutMinDate);

                var dateDifference = daysdifference(selectedDate, $("#checkOutDatepicker").val().replace(/-/g, ","));

                $('#sr_application_form input[name="numberOfNights"]').val(dateDifference);
        ...
        $("#checkOutDatepicker").datepicker({
            dateFormat: 'yy-mm-dd',
            minDate: checkOutMin,
            maxDate: new Date("{{ $tournament->default_check_out }}".replace(/-/g, ",")),
            onSelect: function(date){
                var selectedDate = new Date(date.replace(/-/g, ","));
                var dateDifference = daysdifference($("#checkInDatepicker").val().replace(/-/g, ","), selectedDate);
...
//console.log(jQuery(selector, element.form).filter(function () { return validator.elementValue(this); }).length);
    ...
    return validOrNot;
            ...
            //4111 1111 1111 1111
            var tr_form = document.getElementById('sr_application_form');
             var v = $("#sr_application_form").validate({
                errorClass: "error",
                rules: {
                    email:{
                        required: true,
                    ...
                    @if ($roomAvailableCount > 1)
                        @foreach( $TournamentHotelRooms as $requestRoom) 
                        "room-{{$requestRoom->id}}": {
            ...
            /*$("#sr_application_form input").blur(function() {
                v.element($(this)); 
                setTimeout(function() {
            ...
            $('input.roomsInput').on('blur keyup', function() {
                @if ($roomAvailableCount == 1)
                    if($('#sr_application_form input[name="{{$roomNameIfOneOnly}}"]').valid()) {
                        validRooms = true;
                    } else {
                        validRooms = false;
                    }
            ...
            function calculatePrice() {
                ...
                var nights = $('#sr_application_form input[name="numberOfNights"]').val();
                console.log(nights);
            ...
            $('input').on('blur keyup', function() {
                console.log($("#sr_application_form").validate().checkForm());

                //console.log($(this).valid());
                //$(this).valid();
                

                if ($("#sr_application_form").validate().checkForm() && document.querySelector("#cccvv .CollectJSValid") !== null && document.querySelector("#ccexp .CollectJSValid") !== null && document.querySelector("#ccnumber .CollectJSValid") !== null) {
            ...
            function teamRequestSubmitForm () {
                ...
                grecaptcha.ready(function() {
                    ...
                    // Add your logic to submit to your backend server here.
                        ...
                        //var teamName = $('#sr_application_form input[name="teamName"]').val();
                        //var teamRequestID = $('#sr_application_form input[name="trID"]').val();
                        var tournamentID = $('#sr_application_form input[name="tID"]').val();
                        var hotelID = $('#sr_application_form input[name="hID"]').val();
                        var contactFirstName = $('#sr_application_form input[name="firstName"]').val();
                        var contactLastName = $('#sr_application_form input[name="lastName"]').val();
                        var phoneNumber = $('#sr_application_form input[name="phone"]').val();
                        var email = $('#sr_application_form input[name="email"]').val();
                        var address = $('#sr_application_form input[name="address"]').val();
                        var zip = $('#sr_application_form input[name="zip"]').val();
                        var city = $('#sr_application_form input[name="city"]').val();
                        var state = $('#sr_application_form #state option:selected').val();
                        //var specialRequest = $('#sr_application_form textarea#specialRequest').val();
                        var numberOfnights = $('#sr_application_form input[name="numberOfNights"]').val();
                        var checkInDate = $('#sr_application_form input[name="checkInDate"]').val();
                        var checkOutDate = $('#sr_application_form input[name="checkOutDate"]').val();
                        
                        var specialRequest = $('#sr_application_form textarea#specialRequest').val();

                        var payment_token = $('#sr_application_form input[name="payment_token"').val();

                        var hotelRoomsData = new Array();

                        //$('#sr_application_form .teamRequestRooms #hotel-' + selectedHotel + ' input[type=number]').hide();

                        $('#sr_application_form #rooms-form-group .form-group input[type=number]').each(function(){
                            console.log($(this).val());
                            hotelRoomsData.push({ 'trRoomID': $(this).data('request-room-id'), 'roomsRequested': $(this).val()});
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            url: "/requests/single-submit-request",
                            success:function(data){
                                //JSON.parse(data);
                                console.log(data);
                                //console.log(data['responseData']);

                                $("#sr_application_form").hide('fade');
